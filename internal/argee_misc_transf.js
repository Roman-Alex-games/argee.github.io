 ...
 ********************************************************************************
 *
 *  DESCRIPTION: AJAX functions used in ARGEE PRO/Flowchart. This file contains functions
 *               which extract the project out of the device, store project file
 *               into the device.  
 *
 *******************************************************************************/
...
function onImportProj(e)
{
    ...
    if (extensions[extensions.length-1].toUpperCase()=="ARG")
    {
        ...
        var arr8=GOM.convArrBufToArr8(e.target.result);
		eraseProject();
		setLocalStorage("simCode",JSON.stringify(arr8));
		TRANSF.DlgHide('Simulation')
    }
}

function pro_mode_click(elem)
{
	default_pro_mode=elem.checked;
}
...
function DlgShow(pg,ip)
{
   ...
   outp="";
	...
	if (query_elems["sim_import"]!=undefined)
	{
		outp+="<br><fieldset><br><legend>Selection Project File for Simulation</legend><table>";
		outp+="<tr>";
		outp+="</tr>"
		outp+="<tr>";
		outp+="<td colspan=\"3\">";
		outp+="<center><input type=\"file\" id=\"files\"  onchange=\"TRANSF.handleFileSelect(this);return false;\" name=\"files[]\" multiple /></center>";
		outp+="</td>";
		outp+="</tr>"
	...
	else
	{
		...
		if (pg==1)
		{
		  ...
		  outp+="<center><input list=\"ip_addresses\" id=\"ip_addr_prompt\" value=\""+ip+"\" oncontextmenu=\"return TRANSF.oncontextMenu(event,this)\" onmousedown=\"return TRANSF.onIPMouseDown(event,this)\" onkeydown=\"return TRANSF.onKeyDown(event,this)\" >"
		  outp+="<datalist id=\"ip_addresses\">";
		  var i,ip_list;
		  ip_list=[];
		  for(i=0;i<4;i++)
		  {
			 if (getLocalStorageIndexed(i,"def_ip")!=undefined)
			 {
				ip_list[ip_list.length]=getLocalStorageIndexed(i,"def_ip");
				outp+="<option value=\""+ip_list[ip_list.length-1]+"\">";
			 }
		  }
		  outp+="</datalist>";
		  outp+="</center>";
		...
		if (pg!=0)	
		{
			outp+="<br><fieldset><legend>Simulation Mode</legend><table>";
			outp+="<tr>";
			outp+="</tr>"
			outp+="<tr>";
			outp+="<td colspan=\"3\">";
			...
			outp+="</tr>"
			outp+="</table></fieldset>";
			outp+="<br><fieldset><table>";
			outp+="<tr>";
			outp+="<td colspan=\"3\">";
			outp+="<center>Pro Mode:<input type=\"checkbox\" checked  name=\"Pro Mode\"  onclick=\"TRANSF.pro_mode_click(this)\"   /></center>";
			outp+="</td>";
			outp+="</tr>"
			outp+="</table></fieldset>";
		}
...
function* startEnvConn()
{
	...
	var d1=new Date();
	var time_diff=d1.getTime()-d.getTime();
	console.log("request time="+time_diff);
	GOM.setSimpleRequestTime(time_diff);
	...
	for(i=0;i<num_mods;i++)
	{
		...
		if (i==0)
		{
			...
			id_list[i]=id_list[i]&0xfffff0ff; // clear GW_ID subvariant
		}
		var slice_obj={};
	...
	if (load_locally==false)
	{
		...
		else
		{
			...
			{
				adjustMenuScreen(1);
				var str=GOM.convArrToUnicodeString(GOM.getObjArrCompressed("ARGEE_SOURCE_CODE",0));
				if (str.length==0)
				{
	...
	// if batch compilation is requested - compile and process next file.
...
function* UploadProg()
{
   ...
   GOM.setObjNum("MBS_CONNECTION_TIMEOUT",0,argee_modbus_tcp_connection_timeout);
	...
	var prj_obj_list_curr=fastClone(getProjObjList());	
	...
	if ((typeof additional_code_segment !== 'undefined')&&(additional_code_segment.LOADABLE_CODE!=undefined))
	{
   	prj_obj_list_curr[prj_obj_list_curr.length]=["PROG_HIGH_SPEED_LOADABLE_SEG_OFFSET_OBJ",0];
   }
...
//	SIM.preprocessProjFile(GOM.createProjFile(prj_obj_list_curr));
	...
	console.log("Project storage started....");
	...
	if (PROCESS.isRT_Enabled()!=rt_state)
	{
		rt_state=PROCESS.isRT_Enabled();
		setCompilerMessage(false,false,"ARGEE Real Time state changed. Wait for 20 seconds before switching to Debug");
		GOM.finAjaxAction();
...
function onIPMouseDown(event,elem)
{
   if (event.button==2)
   {
      elem.value="";
...
function oncontextMenu(event,elem)
{
   event.preventDefault();
   return false;
}

function getProjObjList()
{
	var prj_obj_list_curr=fastClone(prj_obj_list);
	if (isEtherCAT(GOM.getObjNum("ARGEE_PROJ_DEV_ORDER_NUM",0))==false)
	{
		prj_obj_list_curr[prj_obj_list_curr.length]=["MBS_CONNECTION_TIMEOUT",0];
	}
	return prj_obj_list_curr;
}
...
return {
	...
	StopProj:StopProj,
	getProjObjList:getProjObjList,
	getIP:getIP,
	startEnvConn:startEnvConn,
	onKeyDown:onKeyDown,
	setIP:setIP,
   onIPMouseDown:onIPMouseDown,
   oncontextMenu:oncontextMenu,
	handleFileSelect:handleFileSelect,
	onImportProj:onImportProj,
	pro_mode_click:pro_mode_click,
}
}());
