...
var ARGEE_nst_code_gen=(function()
	{ 
		...
		var loadable_module_offset=0xffffffff;
		function generateAsm(ast)
		{
			...
			loadable_module_offset=0xffffffff;
			...
			if ((typeof additional_code_segment !== 'undefined')&&(additional_code_segment.LOADABLE_CODE!=undefined))
			{
				...
				loadable_module_offset=rel_pc;
				for(i=0;i<additional_code_segment.LOADABLE_CODE.length;i++)
				{
					compiled_program_DataView.setUint8(rel_pc,additional_code_segment.LOADABLE_CODE[i]);rel_pc++;
				}
			}
		...
		function printCompiledToStr()
		{
			...
			for(i=0;i<asm_descr.length;i++)
			{
				...
				if (asm_descr[i].obj!=undefined)
				{
					str+=parse.quickPrintASTtoStr(asm_descr[i].obj);
				}
				if (asm_descr[i].len==2)
				{
					asm_enc=pad(compiled_program_DataView.getUint16(asm_descr[i].pos,true).toString(16),4);
		...
		*/
		...
		function JumpInstTraceWithAST(ast,check_r0)
		{
			...
			if ((arm7_mode==false)&&(force_convert_expression_result_to_1_bit==true))
			{
		...
		function UpdateControlSegObjs()
		{
			...
			GOM.setObjNum("PROG_HIGH_SPEED_LOADABLE_SEG_OFFSET_OBJ",0,loadable_module_offset);
		}
		...
		*/
		...
		var asm_ast_output=null;
		
		function getAsmOutput()
		{
			return asm_ast_output;
		}
		...
		function generate(ast)
		{
			...
			asm_ast_output+=parse.quickPrintASTtoStr(ast)
			globals=ast;
			...
			try
			{
				...
				asm_ast_output+=printCompiledToStr();
