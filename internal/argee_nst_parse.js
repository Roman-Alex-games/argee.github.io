		...
		else if (next_token==token_types.ENUM_BEGIN)
		{
			...
			while (next_token!=token_types.ENUM_END)
			{
				...
				if (next_token==token_types.COLUMN)
				{
					...
					if (next_token==token_types.FLOAT_CONST)
					{
						enum_elem.float_const=true;
						var obj_const=new Object();
						setObjType(obj_const,"EXPR_VAR",ELEM.EXPR_VAR);
						obj_const.list=[];
						var obj1=new Object();
						setObjType(obj1,"EXPR_VAR",ELEM.EXPR_VAR);
						obj1.data=getConstFloatName(next_token_suppliment);
						...
						obj1.const_val=next_token_suppliment;
						obj1.const_elem=true;
						obj_const.list[0]=obj1;
						enum_elem.ref_float_const_var=obj_const;
					}
					getNextToken();
				}
				...
				else
				{
				   ...
				   skipHMI_Comments();
					...
					else if (next_token==token_types.HMI_GRID_SCREEN)
					{
						...
						{
							var row;
							screen.list[screen.list.length]=parseGrid_HMI_Row();
...
function pasetGrid_HMI_ColElement()
{
		if (next_token==token_types.COMMENT_BEGIN)
		{
			var obj1=new Object();
			setObjType(obj1,"COMMENT_BLOCK",ELEM.COMMENT_BLOCK);
			obj1.list=[];
			getNextToken();	
			obj1.list[0]=pasetGrid_HMI_ColElement();
		...
		{
			var obj1=parseVariableOrFunction(null,true,true);
			match(token_types.SEMICOL);
			return obj1;
		}
		else
...
function parseGrid_HMI_Col()
{
	...
	{
		section.list[section.list.length]=pasetGrid_HMI_ColElement();
	}
	return section;
...
function parseGrid_HMI_Row()
{
	...
	while(next_token!=token_types.END_HMI_GRID_ROW)
	{
		...
		row.list[row.list.length]=parseGrid_HMI_Col();
		getNextToken(); 
	}
...
function parseHMI_BlockElements()
{
	...
	while((next_token!=token_types.END_HMI_BLOCK)&&(next_token!=token_types.END_HMI_BLOCK_SCREEN))
	{
		if (next_token==token_types.COMMENT_BEGIN)
		{
			...
			obj1.list=[];
			getNextToken();	
			obj1.list[0]=parseHMI_BlockElements();
			obj.list[i]=obj1;
		}
		else if (next_token==token_types.HMI_BLOCK)
		{
			getNextToken();	
			var obj_tmp=parseHMI_Block();
			obj.list[i]=obj_tmp;
		}
		else if (next_token==token_types.COMMENT_END)
		{
			break;
		}
		else
...
function parseInitSequence(varPart)
{
	var assignment_list=[];
	var array_element_list=[];
	var i;
	if (next_token==token_types.LEFT_SQ_BRACKET)
	{
		getNextToken();
		while(1)
		{
			var_mode_assignment=true; // hack for handling strings 
			array_element_list[array_element_list.length]=parseExpression();
			var_mode_assignment=false;
			if (next_token==token_types.RIGHT_SQ_BRACKET)
			{
				getNextToken();
		...
		// create a sequence of FUNC_BLOCK_VAR_ASSIGN statements where each will 
		// asisgn a single index
		if (varPart.list[varPart.list.length-1].enum_type!=ELEM.EXPR_VAR_ARR_IND)
		{
			stopCompilation("Array Index required as a starting offset for assignment",varPart);
		}
		// make sure that the data is constant
		var charcode=varPart.list[varPart.list.length-1].list[0].data.charCodeAt(0);
		if ((charcode<0x30)||(charcode>0x39))
		{
			stopCompilation("Array Index is not a number",varPart);
		}
		var starting_offset=parseInt(varPart.list[varPart.list.length-1].list[0].data);
		for(i=0;i<array_element_list.length;i++)
		{
			var obj=new Object();
			setObjType(obj,"FUNC_BLOCK_VAR_ASSIGN",ELEM.FUNC_BLOCK_VAR_ASSIGN);
			obj.list=[];
			obj.list[0]=fastClone(varPart);
			obj.list[0].list[obj.list[0].list.length-1].list[0].data=(starting_offset+i)+"";
			obj.list[0].list[obj.list[0].list.length-1].list[0].elem_string=obj.list[0].list[obj.list[0].list.length-1].list[0].data;
			// modify the array index to point to i
			// make sure that the last element contains array_index
			obj.list[1]=array_element_list[i];
			assignment_list[i]=obj;
		}
	}
	else
	{
...
/*
	...
	else if (next_token==token_types.ASSIGNMENT)
	{
		...
		if (next_token==token_types.LEFT_ROUND_BRACKET)
		{
			...
			getNextToken();
			var init_list_text_pos=[];
			init_list_text_pos[0]=token_start_line;
			init_list_text_pos[1]=token_start_pos;
			
			while(1)
			{
				// if first element is = -> simple assignment
				if (next_token==token_types.EQ)
				{
					// simple asisgnment
					var obj2=new Object();
					...
					obj2.list=[];
					getNextToken();
					// override the initial token strt becuase we want to ignore the leading "="
					init_list_text_pos[0]=token_start_line;
					init_list_text_pos[1]=token_start_pos;
					
					var seq_list=parseInitSequence(obj2);
					var i;
					for(i=0;i<seq_list.length;i++)
					{
						act_obj_type.initialize_data[act_obj_type.initialize_data.length]=seq_list[i];
					}
					init_list_text_pos[2]=token_start_line;
					init_list_text_pos[3]=token_start_pos;
					match(token_types.RIGHT_ROUND_BRACKET);
					break;
				}
				else if ((next_token==token_types.IDENT)||(next_token==token_types.LEFT_SQ_BRACKET))
				{
					// array element or member based innitialization
					// member based initialization
					var var_part=parseVarPart();
					if ((next_token==token_types.EQ)||(next_token==token_types.ASSIGNMENT))
					{
						getNextToken();
					}
					else
					{
						stopCompilation("No Assignment statement in initialization");
					}
					
					var seq_list=parseInitSequence(var_part);
					var i;
					for(i=0;i<seq_list.length;i++)
					{
						act_obj_type.initialize_data[act_obj_type.initialize_data.length]=seq_list[i];
			...
			act_obj_type.init_list_expr_text=extractExpression(init_list_text_pos,true).trim();
			
		}
		else if (((act_obj_type.type==ELEM.VAR_ARR)&&(act_obj_type.data=="CHAR"))||
...
function parseStatements(inject_preemption,inject_return)
{
	...
	while(1)
	{
		...
		if (next_token==token_types.IDENT)
		{
			var ident_start_line=token_start_line;
			var ident_start_pos=token_start_pos;
			...
			else if (next_token==token_types.LEFT_ROUND_BRACKET)
			{
				...
				// this could be a function call or function_block call
					...
					{
						pos_data=[ident_start_line,ident_start_pos,end_expr_line,end_expr_pos];
						obj1.elem_string=extractExpression(pos_data,true)+")";
					}
					else
					{
						pos_data=[start_expr_line,start_expr_pos,end_expr_line,end_expr_pos];
...
function getStringFromAST_StrippedSpecialComments(ast)
{
	var states=
	{
		ST_NORM:0,
	...
	var str=getStringFromAST(ast);
	var i;
	var curr_state=states.ST_NORM;
	...
	for(i=0;i<str.length;i++)
	{
		switch(curr_state)
		{
			case states.ST_NORM:
			{
				if (char_arr[i]=='{')
				{
					curr_state=states.ST_CURVY_BRACKET;
				}
				else
				{
					outp_arr[outp_arr.length]=char_arr[i];
				}
				break;;
			}
			case states.ST_CURVY_BRACKET:
			{
				if (char_arr[i]=='[')
				{
					curr_state=states.ST_SQ_BRACKET;
				}
				else
				{
					outp_arr[outp_arr.length]='}';
					outp_arr[outp_arr.length]=char_arr[i];
					curr_state=states.ST_NORM;
				}
				break;
			}
			case states.ST_SQ_BRACKET:
			{
				if (char_arr[i]=='}')
				{
					curr_state=states.ST_NORM;
	...
	// scan the string and remove inner comments which start with {[....]}
	str=outp_arr.join('');
...
function displayAST_Elem(indent_level,ast,option)
{
	...
	switch(ast.enum_type)
	{
		...
		case ELEM.ELEM_FUNC_BLOCK_PROLOGUE:
			...
			ast_disp_output[curr_ast_disp_line]+="FUNCTION_BLOCK_PROLOGUE "+ast.parent_node.parent_node.name;
			curr_ast_disp_line++;
			break;
		...
		case ELEM.FUNC_BLOCK_ARG_LIST:
			for(i=0;i<ast.list.length;i++)
			{
				if (ast.list[i].enum_type==ELEM.FUNC_BLOCK_VAR_ASSIGN)
				{
					displayAST_Elem(indent_level,ast.list[i].list[0],option);
					ast_disp_output[curr_ast_disp_line]+=":=";
...
return {
   ...
   getStringFromAST_StrippedSpecialComments:getStringFromAST_StrippedSpecialComments,
   quickPrintASTtoStr:quickPrintASTtoStr,
}
