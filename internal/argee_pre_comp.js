	...
	for(prop in elem)
	{
			...
			if (elem.type==block_types.comment_var)
			{
				if (parent.type==block_types.alias_vars)
				{
					elem.values[1]="1";
						 ...
						 (parent.type==block_types.module_variables))
				{
					elem.values[1]="0";
				}
				var jk=1;
		...
		else
		{
			...
			if ((elem.type==block_types.reg_var)&&(elem.sub_elems.length>0))
			{
				...
				obj1.sub_elems[0]=fastClone(elem.sub_elems[0]);
				// if no "=" present -> simple initialization ->add = in front
				if (obj1.sub_elems[0].values[0].indexOf("=")==-1)
				{
					obj1.sub_elems[0].values[0]="="+obj1.sub_elems[0].values[0];
				}
				obj1.sub_elems[0].type=block_types.init_block_simp;
			...
			else if (elem.type==block_types.funct_block_var)
			{
				...
				if (obj.values[1]==func_block_type_num.TASK)
				{
					obj.type=block_types.conv_task_func_block; 
				}
				else if (obj.values[1]==func_block_type_num.HMI_BLOCK)
				{
					obj.type=block_types.conv_hmi_block_func_block; 
				}
				else if (obj.values[1]==func_block_type_num.HMI_CONTROL)
				{
					obj.type=block_types.conv_hmi_control_func_block; 
				}
				else if (obj.values[1]==func_block_type_num.BITFIELD)
				{
					obj.type=block_types.conv_bitfield_func_block;
				}
				else if (obj.values[1]==func_block_type_num.FUNCTION)
				{
					obj.type=block_types.conv_function_func_block;
				}
				
				else if (obj.values[1]==func_block_type_num.PROCEDURE)
				{
					obj.type=block_types.conv_procedure_func_block;
				}
				
				else if (obj.values[1]==func_block_type_num.STRUCT)
				{
					obj.type=block_types.conv_struct_func_block;
				}				
				else
				{
				...
				var j;
				var arg_list="";
				if (obj.type!=block_types.conv_bitfield_func_block)
				{
				...
				for(i=1;i<elem.sub_elems.length;i++)
				{
					...
					else
					{
						...
						if (obj.type!=block_types.conv_bitfield_func_block)
						{
							type=num(elem.sub_elems[i].values[2]);
						...
						else
						{
							...
							obj.sub_elems[type].sub_elems[obj.sub_elems[type].sub_elems.length-1].values[1]="BITS_"+(parseInt(obj.sub_elems[type].sub_elems[obj.sub_elems[type].sub_elems.length-1].values[2])+1);
						}
			...
			else
			{
				...
				for(i=0;i<elem.sub_elems.length;i++)
				{
					...
					else
					{
						...
						if ((obj.sub_elems[obj.sub_elems.length-1].type==block_types.reg_var)&&
							(obj.sub_elems[obj.sub_elems.length-1].sub_elems!=undefined)&&
							(obj.sub_elems[obj.sub_elems.length-1].sub_elems.length>0))
						{
							// check if this is the reg_var and with initializations -> change type to conv_reg_var_with_init
							// This needs to be done here after the type sustitution
							//elem.type=block_types.conv_reg_var_with_init;	
							var jk=1;
							obj.sub_elems[obj.sub_elems.length-1].type=block_types.conv_reg_var_with_init;
						}
...
var func_block_type_num=ARGEE_elem_descr.func_block_type_num;
...
[
	...
	{type:block_types.init_block_simp,arr:[[0],[]]},
	...
	{type:block_types.module_variables,arr:[["VAR_MODULE"],["END_VAR"]]},
	...
	// semicolumn is added in the initialization segment
	{type:block_types.conv_reg_var_with_init, second_format_adder:-1,format_switch_cond_value:2,switch_threshold:0,arr:[[0,":",1],[],[0,": ARRAY [0..",2,"] OF ",1],[]]},
...
//{type:block_types.plc_var,format_switch_cond_value:3,switch_threshold:0,arr:[[0,": WORD AT PLC:",1,":",2,";"],[],[0,": ARRAY [0..",3,"] OF WORD AT PLC:",1,":",2,";"],[]]},
	...
	{type:block_types.conv_hmi_block_func_block,arr:[["HMI_BLOCK_FUNCT_BLOCK ",0,"(",2,")"],["END_FUNCTION_BLOCK"]]},	

	{type:block_types.conv_hmi_control_func_block,arr:[["HMI_CONTROL_FUNCT_BLOCK ",0,"(",2,")"],["END_FUNCTION_BLOCK"]]},	

	{type:block_types.conv_bitfield_func_block,arr:[["BITFIELD_FUNCT_BLOCK ",0,"(",2,")"],["END_FUNCTION_BLOCK"]]},	
	...
	{type:block_types.conv_struct_func_block,arr:[["STRUCT ",0,"(",2,")"],["END_STRUCT"]]},	
	{type:block_types.conv_function_func_block,arr:[["FUNCTION ",0,"(",2,")"],["END_FUNCTION"]]},	
	{type:block_types.conv_procedure_func_block,arr:[["PROCEDURE ",0,"(",2,")"],["END_PROCEDURE"]]},		
	...
	{type:block_types.hmi_block,arr:[["HMI_BLOCK ",0,";"],["END_HMI_BLOCK"]]},
	{type:block_types.hmi_block_screen,arr:[["HMI_BLOCK_SCREEN ",0,";"],["END_HMI_BLOCK_SCREEN"]]},
	{type:block_types.hmi_control,arr:[[0,";"],[]]},
...
function handleElemConv(elem,start,indent,mode)
{
	...
	if ((rule=rule_lookup[elem.type])!=undefined)
	{
		...
		{
			var num_elems,num_str;
			num_elems=parseInt(elem.values[rule.format_switch_cond_value]);
			num_str=null;
			if ( Number.isNaN(num_elems))
			{
				if (elem.values[rule.format_switch_cond_value].length>0)
				{
					num_elems=2;
					num_str=elem.values[rule.format_switch_cond_value];
				}
			}
			if (start==false)
			{
				...
				{
					format=rule.arr[3];
					elem.values[rule.format_switch_cond_value]=num_str+"-";
				}
			}
			else
			{
				...
				{
					format=rule.arr[2];
					elem.values[rule.format_switch_cond_value]=num_str+"-1";
				}
			}
		}	
		else
...
function handleElem(elem,indent,mode)
{
	var str="";
	var i;
	var updated_indent=indent;
	if ((elem.type==block_types.conv_hmi_control_func_block)||	
		(elem.type==block_types.conv_hmi_block_func_block))
	{
		// for HMI function block -> don't add comments
		mode=0;	
	}
	if ((elem.type==block_types.conv_if_then)||	
		(elem.type==block_types.conv_else)||	
		(elem.type==block_types.conv_else_if))
	{
		updated_indent--;	
	}
	...
	str+=handleElemConv(elem,true,updated_indent,mode);
	if (elem.sub_elems!=undefined)
	{
		for(i=0;i<elem.sub_elems.length;i++)
		{
			str+=handleElem(elem.sub_elems[i],updated_indent+1,mode);
		}
	}
	str+=handleElemConv(elem,false,updated_indent,mode);
