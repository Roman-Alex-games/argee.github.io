 ...
 ********************************************************************************
 *
 *  DESCRIPTION: IO Configuration editor. This code renders the IO configuration
 *               screens out of the GOM objects in the ARGEE environment.
 *               When user modifies the value of the parameter - it is stored in 
 *               appropriate GOM objects.
 *
 *******************************************************************************/
...
var IO_Config = (function ()
{
	...
	function GetArrValue_ext(dt, offset, len,signed)
	{
	   var val=0;
	...
	function SetArrValue_ext(dt,offset,len,val,signed)
	{
	  var byte_num=(offset/8) >> 0;
	  var bit_num=offset%8;
	  if ((len%8)==0)
	  {
		 switch(len)
		 {
			case 8:
				if (signed==false)
				{
					dt.setUint8(byte_num,val);
				}
				else
				{
					dt.setInt8(byte_num,val);
				}
			   break;
			case 16:
				if (signed==false)
				{
					dt.setUint16(byte_num,val,true);	
				}
				else
				{
			...
			case 32:
   				if (signed==false)
				{
					dt.setUint32(byte_num,val,true);	
				}
				else
				{
...
*/
		...
		for (var j = 0; j < slices[slot].sections[param_sect].objects.length; j++)
		{
			...
			if ((slices[slot].sections[param_sect].objects[j].dataType == IODBDataType.INT)||
				(slices[slot].sections[param_sect].objects[j].dataType == IODBDataType.INT_S8)||
				(slices[slot].sections[param_sect].objects[j].dataType == IODBDataType.INT_S16)||
				(slices[slot].sections[param_sect].objects[j].dataType == IODBDataType.INT_S16))
			
			{
				...
				HTMLContent += BEP_RenderIntObject(slotNum, param_sect, slices[slot].sections[param_sect].objects[j].offset, slices[slot].sections[param_sect].objects[j].length, slices[slot].sections[param_sect].objects[j].name,dv,signed);
			}
	...
	//*********************************************************************************
	function BEP_RenderIntObject(slotNum, dataSect, bitOffset, bitLen, dataPointName,dv,signed)
	{
		var objDataValue;
		var objDataByteLen;
		var objName = "";
		...
		objDataValue = ToInteger(GetArrValue_ext(dv, bitOffset, bitLen,signed));
		if (rw_render_mode==false)
		{
	...
	function handleChange()
	{
		...
		for (var i = 0; i < formElems.length; i++)
		{
			...
			if ((formElems[i].id.substring(0, 1) != "_") && (arrList.length == 4))
			{
				...
				var signed=false;
				if ((dataType==IODBDataType.INT_S8)||(dataType==IODBDataType.INT_S16)||(dataType==IODBDataType.INT_S32))
				{
					signed=true;
				}

				// modify data
				SetArrValue_ext(dv, bitOffset, bitLen, val,signed);
			}
		}
