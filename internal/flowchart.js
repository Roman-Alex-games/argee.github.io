...
{
	flowchart=JSON.parse(GOM.convArrToUnicodeString(GOM.getObjArrCompressed("ARGEE_SOURCE_CODE",0)));
}
...
function generateARGEE_Code(upload)
{
	...
	func_blocks[1].values[1]=DESCR.func_block_type_num.TASK;
	...
	if (glob_libs_uncompressed.length>0)
	{
		IMP.import_proj(glob_libs_uncompressed,false,true,false);
	}
	var st;
	st=ARGEE_pre_comp.preCompile(0);
	if ((upload==false)&&(default_pro_mode==true))
	{
		GOM.setObjArrCompressed("ARGEE_SOURCE_CODE",0,GOM.convUnicodeStringToArr(st));
	}
	else
	{
		GOM.setObjArrCompressed("ARGEE_SOURCE_CODE",0,GOM.convUnicodeStringToArr(JSON.stringify(flowchart)));
	}
...
function convertToArgee_imp(render)
{
	generateARGEE_Code(false);
	editor="regular";
...
function convertToArgee()
{
	...
	return convertToArgee_imp(true);
}
...
return {
	...
	convertToArgee_imp:convertToArgee_imp,
}
}());
