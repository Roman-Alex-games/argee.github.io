 ...
 ********************************************************************************
 *
 *  DESCRIPTION: Importer. Imports ARGEE2 and ARGEE3 project files into 
 *               environment and performs nessesary conversions to display them
...
function extractFunctionBlockAndModuleNames(prj)
{
	var i;
	for(i=0;i<prj.list[1].list.length;i++)
	{
		...
		if (prj.list[1].list[i].enum_type==ELEM.MODULE)
		{
			module_info[module_info.length]={name:prj.list[1].list[i].name,version:prj.list[1].list[i].version};	
		}
	}
}
...
function preProcessVars(var_segm)
{
	...
	for(i=0;i<var_segm.list.length;i++)
	{
		...
		{
			if (var_segm.list[i].type.range_end==-1)
			{
				var_list[curr].num_elems=var_segm.list[i].type.range_end_named_const;
			}
			else
			{
		...
		if (var_list[curr].type_ind!=-1)
		{
			var_list[curr].act_st_type=updated_type_enum[var_list[curr].type_ind];
		}
		else
		{
			var_list[curr].act_st_type=var_segm.list[i].type.data;
		}
		...
		if (var_segm.list[i].type.initialize_data!=undefined)
		{
			...
			var str_with_inits="";
			if (var_segm.list[i].type.init_list_expr_text!=undefined)
			{
				str_with_inits=var_segm.list[i].type.init_list_expr_text;
			}
			else
			{
				var j;
				...
				for(j=0;j<var_segm.list[i].type.initialize_data.length;j++)
				{
					...
					if (var_segm.list[i].type.initialize_data[j].list[0].list.length==0)
					{
						str_with_inits+=val_str;
					}
					else
					{
						str_with_inits+=ARGEE_nst_parse.getStringFromAST(var_segm.list[i].type.initialize_data[j].list[0])+"="+val_str;
					}
					//var_list[curr].init_data[j]=[ARGEE_nst_parse.getStringFromAST(var_segm.list[i].type.initialize_data[j].list[0]),val_str];
				}
			}
			var_list[curr].init_data[0]=str_with_inits;
		}
	}
	return var_list;
}
...
function createHMI_BlockSegm(argee_seg,ast,parent_ast,index_ast)
{
	var i;
	for	(i=0;i<ast.list.length;i++)
	{
		if (ast.list[i].data=="COMMENT")
		{
			argee_seg.splice(argee_seg.length-1,0,ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.comment)));
			var curr_hmi_ctrl=argee_seg[argee_seg.length-2];
			curr_hmi_ctrl.values[0]=decodeURI(ast.list[i].list[0].data);
		}

		else if (ast.list[i].enum_type==ELEM.FUNC_NO_RETURN)
		{
			argee_seg.splice(argee_seg.length-1,0,ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.hmi_control)));
			var curr_hmi_ctrl=argee_seg[argee_seg.length-2];
			curr_hmi_ctrl.values[0]=ast.list[i].elem_string;
		}
		else if (ast.list[i].enum_type==ELEM.COMMENT_BLOCK)
		{
			var jk1=1;
			createHMI_BlockSegm(argee_seg,ast.list[i].list[0]);
			var curr_hmi_block=argee_seg[argee_seg.length-2];
			DESCR.setCommentedState(curr_hmi_block,1);
		}
		else if (ast.list[i].enum_type==ELEM.HMI_BLOCK)
		{
			argee_seg.splice(argee_seg.length-1,0,ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.hmi_block)));
			var curr_hmi_block=argee_seg[argee_seg.length-2];
			curr_hmi_block.values[0]=ast.list[i].list[0].elem_string;
			createHMI_BlockSegm(curr_hmi_block.sub_elems,ast.list[i].list[1]);
...
function createCodeSegment(argee_seg,ast,parent_ast,index_ast)
{
	...
	if (ast.enum_type==ELEM.IF)
	{
		...
		argee_seg.sub_elems[argee_seg.sub_elems.length-1].values[0]=ARGEE_nst_parse.getStringFromAST_StrippedSpecialComments(ast.list[0]);
		...
		for(i=2;i<ast.list.length;i+=2)
		{
			...
			else
			{
				...
				argee_seg.sub_elems[argee_seg.sub_elems.length-1].sub_elems.splice(0,1);
				argee_seg.sub_elems[argee_seg.sub_elems.length-1].values[0]=ARGEE_nst_parse.getStringFromAST_StrippedSpecialComments(ast.list[i]);
				createCodeSegment(argee_seg.sub_elems[argee_seg.sub_elems.length-1],ast.list[i+1],ast,i);
				argee_seg.sub_elems[argee_seg.sub_elems.length-1].sub_elems.splice
					  (argee_seg.sub_elems[argee_seg.sub_elems.length-1].sub_elems.length,0,
					   ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.add_block))
	...
	else if (ast.enum_type==ELEM.COMMENT)
	{
		if (ast.preproc_elem!=undefined)
		{
			...
			argee_seg.sub_elems[argee_seg.sub_elems.length-1].values[0]="GENERATE_PREPROC(#\r\n"+ast.data+"##)";
		}
		else
		{
	...
	{
		if (function_block_task==true) {complex_mode=true;}
		var str=ARGEE_nst_parse.getStringFromAST_StrippedSpecialComments(ast);
...
function createRegVarElem(parse_elem,gui_proj_elem_parent)
{
	if (parse_elem.comment!=undefined)
	{
		gui_proj_elem_parent.sub_elems.splice(gui_proj_elem_parent.sub_elems.length-1,
															 0,ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.comment_var)));
		gui_proj_elem_parent.sub_elems[gui_proj_elem_parent.sub_elems.length-2].values[0]=parse_elem.comment;
	}
	else
	{
			// regular var
			...
			gui_proj_elem_parent.sub_elems[gui_proj_elem_parent.sub_elems.length-2].values[0]=parse_elem.name;													 
			gui_proj_elem_parent.sub_elems[gui_proj_elem_parent.sub_elems.length-2].values[1]=updated_type_enum[parse_elem.type_ind];			
			gui_proj_elem_parent.sub_elems[gui_proj_elem_parent.sub_elems.length-2].values[2]=parse_elem.num_elems;
			if (parse_elem.init_data!=undefined)
			{
				gui_proj_elem_parent.sub_elems[gui_proj_elem_parent.sub_elems.length-2].sub_elems=[];
				var ii=0;
				//for(ii=0;ii<parse_elem.init_data.length;ii++)
				{
					gui_proj_elem_parent.sub_elems[gui_proj_elem_parent.sub_elems.length-2].sub_elems[ii]=
					                                                ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.init_elem_var));
					gui_proj_elem_parent.sub_elems[gui_proj_elem_parent.sub_elems.length-2].sub_elems[ii].values[0]=parse_elem.init_data[0];
				}
			}
...
function import_proj(text,storeLocally,module,preview_mode)
{
	...
	var prj=ARGEE_nst_parse.parseProjectOnePass(text,false);
	if (prj==null)
	{
		return null;
	}
	...
	compl_types=[];
	module_info=[];
	extractFunctionBlockAndModuleNames(prj);
	...
	{
		jk=1;
		var unique_proj_names=DESCR.getEnumAndGlobals();
		// check module conflicts
		for(i=0;i<ARGEE_elem_descr.proj_elems.sub_elems.length;i++)
		{
			if (ARGEE_elem_descr.proj_elems.sub_elems[i].type==block_types.module)
			{
				var j;
				for(j=0;j<module_info.length;j++)
				{
					if (module_info[j].name.toUpperCase()==ARGEE_elem_descr.proj_elems.sub_elems[i].values[0].toUpperCase())
					{
						alert("Same module '"+module_info[j].name+"' is present in the project(V"+ARGEE_elem_descr.proj_elems.sub_elems[i].values[1]+" in the project and V"+module_info[j].version+" in the file).  Please unload the module from the project and retry the import");
						return null;
					}
				}
			}		
		...
		// check function block conflicts	
		...
		for(i=0;i<prj.list[1].list.length;i++)
		{
			if (prj.list[1].list[i].enum_type==ELEM.ENUM_OBJ)
			{
				...
				{
					if (unique_proj_names.indexOf(prj.list[1].list[i].list[ii].name)!=-1)
					{
	...
	for(i=0;i<lst.length;i++)
	{
		...
		else if (lst[i].comment!=undefined)
		{
			...
			{
				// prog_var
				createRegVarElem(lst[i],prj_elems.sub_elems[0].sub_elems[0]);
			}
		}
		else
		{
	...
	if (module==true)
	{
		...
		prj_elems.sub_elems[curr_module].values[0]=prj.list[1].list[0].name;
		prj_elems.sub_elems[curr_module].values[1]=prj.list[1].list[0].version;		
	}
	...
	for(i=0;i<prj.list[1].list.length;i++)
	{
		if ((prj.list[1].list[i].enum_type==ELEM.MODULE)&&(i>0)) // new module
		{
			prj_elems.sub_elems.splice(prj_elems.sub_elems.length-2,0,
			                                           ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.module)));
			curr_module++;
			prj_elems.sub_elems[curr_module].values[0]=prj.list[1].list[i].name;
			prj_elems.sub_elems[curr_module].values[1]=prj.list[1].list[i].version;
		}
		else if (prj.list[1].list[i].enum_type==ELEM.VAR_SEGM)
		{
			prj_elems.sub_elems[curr_module].sub_elems.splice(prj_elems.sub_elems[curr_module].sub_elems.length-1,0,
			                                           ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.module_variables)));
			

			var lst=preProcessVars(prj.list[1].list[i]);
			var parent_elem=prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2];
			parent_elem.sub_elems.splice(0,1);
			...
			for(ii=0;ii<lst.length;ii++)
			{
				...
				{
						// prog_var
					createRegVarElem(lst[ii],parent_elem);
				}
				else
				{
					// regular var
		...
		else if (prj.list[1].list[i].enum_type==ELEM.ENUM_OBJ)
		{
			prj_elems.sub_elems[curr_module].sub_elems.splice(prj_elems.sub_elems[curr_module].sub_elems.length-1,0,
			                                           ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.enum_var)));
			var curr=prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2];
				 ...
				 )
		{
			...
			prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[0]=prj.list[1].list[i].name;
			if (prj.list[1].list[i].enum_type==ELEM.TASK)
			{
				if (prj.list[1].list[i].name.toUpperCase()!="DEFAULT_TASK_1")
				{
				}
				prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.TASK;	
			}
			else if (prj.list[1].list[i].enum_type==ELEM.FUNCT_BLOCK)
			{
				if (prj.list[1].list[i].HMI_BLOCK_FUNCT_BLOCK!=undefined)
				{
					prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.HMI_BLOCK;	
				}
				else if (prj.list[1].list[i].HMI_CONTROL_FUNCT_BLOCK!=undefined)
				{
					prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.HMI_CONTROL;	
				}
				else if (prj.list[1].list[i].BITFIELD_FUNCT_BLOCK!=undefined)
				{
					prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.BITFIELD;	
				}
				else if (prj.list[1].list[i].STRUCT!=undefined)
				{
					prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.STRUCT;	
				}
				else if (prj.list[1].list[i].FUNCTION!=undefined)
				{
					prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.FUNCTION;	
				}
				else if (prj.list[1].list[i].PROCEDURE!=undefined)
				{
					prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.PROCEDURE;	
				}
				else
				{
					prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].values[1]=ARGEE_elem_descr.func_block_type_num.REG;	
				}
			}
			if (prj.list[1].list[i].list[1]!=undefined)
			{
				prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].sub_elems[0].sub_elems.splice(0,1);
				...
				createCodeSegment(prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].sub_elems[0],prj.list[1].list[i].list[1]);
				prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].sub_elems[0].sub_elems.splice
				              (prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].sub_elems[0].sub_elems.length,0,
							   ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.add_block))
							  );
			}
			if (prj.list[1].list[i].enum_type!=ELEM.FUNCTION_BLOCK_GROUP)
			{
				...
				prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].
										   sub_elems.splice(1,1);
				var curr_var_list=prj_elems.sub_elems[curr_module].sub_elems[prj_elems.sub_elems[curr_module].sub_elems.length-2].sub_elems;						   
				for(ii=0;ii<prj.list[1].list[i].list[0].list.length;ii++)
				{
					...
					else
					{
						...
						if (prj.list[1].list[i].BITFIELD_FUNCT_BLOCK!=undefined)
						{
							var substr=lst[ii].act_st_type.split("BITS_");
							
							curr_var_list[curr_var_list.length-2].values[1]="Number";
							curr_var_list[curr_var_list.length-2].values[2]=parseInt(substr[1])-1;
							curr_var_list[curr_var_list.length-2].values[3]=lst[ii].num_elems;
						}
						else
						{
							curr_var_list[curr_var_list.length-2].values[1]=lst[ii].act_st_type;
	...
	if (module==false)
	{
		if (prj.hmi!=undefined)
		{
			...
			var curr_screen_list=prj_elems.sub_elems[curr_module].sub_elems[0].sub_elems[0].sub_elems;
			...
			{
				if (prj.hmi.list[i].enum_type==ELEM.HMI_BLOCK)
				{
					curr_screen_list.splice(curr_screen_list.length-1,0,ARGEE_elem_descr.createNewElem(ARGEE_elem_descr.findBlockDescr(block_types.hmi_block_screen)));
					var curr_hmi_block=curr_screen_list[curr_screen_list.length-2];
					
					curr_hmi_block.values[0]=prj.hmi.list[i].list[0].elem_string;
					createHMI_BlockSegm(curr_hmi_block.sub_elems,prj.hmi.list[i].list[1]);
					
					
				}
				else if (prj.hmi.list[i].enum_type==ELEM.HMI_SCREEN)
				{
				...
				else if (prj.hmi.list[i].enum_type==ELEM.HMI_GRID_SCREEN)
				{
					...
					{
						// Use the approach where we extract the "commented out block" and place it into the upper array.
						// mark that element as commented out and in the end of the processing code -> call the DESCR.setCommentedState
						var commented=false;
						if (row_list[ii].enum_type==ELEM.COMMENT_BLOCK)
						{
							row_list[ii]=row_list[ii].list[0];
							commented=true;
						}
						if (row_list[ii].data=="COMMENT")
						{
							...
							if (commented==true)
							{
								DESCR.setCommentedState(act_sect,1);
							}
							continue;
						}
						...
						{
							var commented=false;
							if (act_sect_list[iii].enum_type==ELEM.COMMENT_BLOCK)
							{
								act_sect_list[iii]=act_sect_list[iii].list[0];
								commented=true;
							}
							...
							{
								var commented=false;
								if (act_elem_list[iiii].enum_type==ELEM.COMMENT_BLOCK)
								{
									act_elem_list[iiii]=act_elem_list[iiii].list[0];
									commented=true;
								}
								...
								if (commented==true)
								{
									DESCR.setCommentedState(act_block,1);
								}
			...
			if (prj_elems.sub_elems[curr_module].sub_elems[0].sub_elems[0].sub_elems.length==1)
			{
				prj_elems.sub_elems[curr_module].sub_elems[0].sub_elems[0].expanded=false;
			}
		}
	}
	...
	*/
	...
	{
		// Module -> append function blocks/modules
		
		
		// append function_block definitions
		for(i=0;i<prj_elems.sub_elems.length;i++)
		{
			var elm=prj_elems.sub_elems[i];
			if (elm.type==block_types.module)
			{
				ARGEE_elem_descr.proj_elems.sub_elems.splice(ARGEE_elem_descr.proj_elems.sub_elems.length-2,0,elm);
			}
		}
	}

	DESCR.setNewTypeEnum(updated_type_enum,false);
