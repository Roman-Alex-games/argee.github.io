...
[
	...
	{name:"TBEN-S1",id:0x01500021,diags:blcen_gw_diags},
...
var sim_devices=
[
	{name:"TBEN-L1-16DXP",slice_list:[0x01510028,0x816D5500]},	
	{name:"TBEN-L4-16DXP",slice_list:[0x01500028,0x816D5500]},		
	{name:"TBEN-L1-8DIP-8DOP",slice_list:[0x01510028,0x615B4400]},
	{name:"TBEN-L4-8DIP-8DOP",slice_list:[0x01500028,0x615B4400]},
	{name:"BLCEN-4M12MT-4AI4AO-VI",slice_list:[0x01500021,0x419B9900]},	
	{name:"BLCEN-4M12MT-4AI4AO-VI-4AI4AO-VI",slice_list:[0x01500021,0x419B9900,0x419B9900]},	
	{name:"BLCEN-8M12LT-4AI4AO-VI-8XSG-P",slice_list:[0x01500021,0x419B9900,39273472]},	
	{name:"BLCEN-8M12LT-4AI-VI-8XSG-P",slice_list:[0x01500021,1098354688,39273472]},
	{name:"BLCEN-16M8LT-8XSG-P-8XSG-P",slice_list:[0x01500021,39273472,39273472]},
	{name:"BLCEN-8M12LT-8XSG-P-8XSG-P",slice_list:[0x01500021,39273472,39273472]},
	{name:"BLCEN-6M12LT-2RFID-S-8XSG-P",slice_list:[0x01500021,0x2179CC00,39273472]},
	{name:"BLCEN-4M12MT-8XSG-P",slice_list:[0x01500021,39273472]},
	{name:"BLCEN-2M12MT-2RFID-S",slice_list:[0x01500021,0x2179CC00]},
	{name:"TBEN-S1-4DIP-4DOP",slice_list:[0x01500021,0x40533300]},
	{name:"TBEN-S1-8DXP",slice_list:[0x01500021,0x60554400]},
	{name:"FEN20-16DXP",slice_list:[0x01500027,0x81695500]},
	{name:"FEN20-4DIP-4DXP",slice_list:[0x01500127,0x41564300]},
]
...
function sim_UploadProg(req_code,req_len,dt,offset)
{
	...
	localStorage.simCode=JSON.stringify(arr_obj);
}
...
function sim_DownloadProg(req_code,req_len,dt,offset)
{
	var arr_obj=[];
	var i;
	if (localStorage.simCode!=undefined)
	{
		arr_obj=JSON.parse(localStorage.simCode);
	}
...
function sim_GetStationConfig(req_code,req_len,dt,offset)
{
	var i;
	var offset=0;
	resp_data=new ArrayBuffer(sim_slices_specified.length*4*2);
	var tmp_dt=new DataView(resp_data);
	...
	// write 0 config data for each slice
	for(i=1;i<sim_slices_specified.length;i++)
	{
		tmp_dt.setUint32(offset,0,true);offset+=4;
	}
}
...
{
	var tmp_dt=GOM_CreateRespArr(4);
	tmp_dt.setUint32(0,0x02030000,true);
}
...
function simDevChanged(elem)
{
	...
	eraseProject();
	localStorage.simDevSelect=elem.selectedIndex;
	...
	flowchart={constants:[0,0],timer_exp:[0,0],counter_preset:[0,0],rungs:[]};
	saveLocal();
	localStorage.firstSimStart=true;
...
function simDevInit()
{
	...
	{
		sim_curr_selected_device=parseInt(localStorage.simDevSelect);
...
{
	var i;
	var html="<center><h2>Select Device to Simulate</h2><br><select onchange=\"simDevChanged(this)\">";
...
function getSegmAddr(var_d)
{
	...
	return {view:sect_dataView,offset:offset,len:len};
}
...
function getValue(var_d)
{
	...
	{
		return GetArrValue(segm_elem.view,segm_elem.offset,segm_elem.len);
	}
}
...
function mod()
{
	...
	{
		stop_program=true;
		stop_reason="division by zero(mod)";
	}
	else
	{
		val_d[0]=(val_d[1]%val_d[0]);
