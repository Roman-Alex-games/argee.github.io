...
var screens=
{
	...
	[
		...
		block_types.hmi_screen_color,
	];
	var hmi_table_sceen_col_blocks=
	[
		block_types.hmi_disp_num,  // displays number,hex, string, array
		block_types.hmi_disp_range,
		//block_types.hmi_disp_hex, // displays numbers and arrays
...
[		
	...
	
	{
		...
		nested_blocks:[block_types.hmi_screen,block_types.hmi_table_screen],
	...
	
	{
		...
		AddButtonText:"Add Section",
	 	nested_blocks:[block_types.hmi_section]
	},

	{
		title:"HMI Table <br> Screen", 
		type:block_types.hmi_table_screen,
		color:"#FF8ee",
		screens:[screens.prog_scr], 
		refresh_screens:[screens.prog_scr], 
		fields:
    	[
			{field_name:"Name",type:field_types.str_num,automatic_var_replacement:false},
			{field_name:"HTML Screen Prefix",type:field_types.str_num,automatic_var_replacement:false,default_val:"<center><div  style='border-radius: 5px;width:90%'>"},
			{field_name:"HTML Screen Suffix",type:field_types.str_num,automatic_var_replacement:false,default_val:"</div></center>"},
		],
		AddButtonText:"Add Table",
	 	nested_blocks:[block_types.hmi_table]
	},

	{
		title:"Table", 
		type:block_types.hmi_table,
		color:"FFfDC",
		screens:[screens.prog_scr], 
		refresh_screens:[screens.prog_scr], 
		fields:
    	[
			{field_name:"HTML Table Prefix",type:field_types.str_num,automatic_var_replacement:false,default_val:""},
			{field_name:"HTML Table Suffix",type:field_types.str_num,automatic_var_replacement:false,default_val:""},
			{field_name:"HTML Table<br> properties",type:field_types.str_num,automatic_var_replacement:false,default_val:"style='table-layout: fixed;width:100%;'"},

		],
		AddButtonText:"Add Row",
		nested_blocks:[block_types.hmi_table_row]
	},

	{
		title:"Table Row", 
		type:block_types.hmi_table_row,
		color:"FFDFC",
		screens:[screens.prog_scr], 
		refresh_screens:[screens.prog_scr], 
		fields:
    	[
			{field_name:"HTML Row<br> properties",type:field_types.str_num,automatic_var_replacement:false,default_val:"",render_name:true},
		],
		AddButtonText:"Add Column",
		nested_blocks:[block_types.hmi_table_col]
	},
	{
		title:"Table Column", 
		type:block_types.hmi_table_col,
		color:"FFCFD",
		screens:[screens.prog_scr], 
		refresh_screens:[screens.prog_scr], 
		fields:
    	[
			{field_name:"HTML Column <br> Properties",type:field_types.str_num,automatic_var_replacement:false,default_val:"",render_name:true},
			{field_name:"HTML Inner <br> Element Prefix",type:field_types.str_num,automatic_var_replacement:false,default_val:""},
			{field_name:"HTML Inner <br> Element Suffix",type:field_types.str_num,automatic_var_replacement:false,default_val:""},
		],
		AddButtonText:"Add Column Element",
		nested_blocks:hmi_table_sceen_col_blocks
	},
	...
	{
		title:"Display Number<br> With Value Range", 
...
/*			{field_name:"Min Value",type:field_types.str_num},
	...
	{
		title:"Screen Color <br>(hidden)", 
		type:block_types.hmi_screen_color,
		color:"#e168a2",
		screens:[screens.prog_scr], 
		refresh_screens:[screens.prog_scr], 
		hmi_func_array:"hmi_disp_screen_color",
		fields:
    	[
			{field_name:"Variable",type:field_types.str_num,render_name:true},
		],
	},
	{
		title:"Screen List", 
	...
	},
	{
		...
		[
			{type:field_types.str_num,field_name:"Timer",default_val:""},
			{type:field_types.str_num,field_name:"Expired(ms)",default_val:"1000"}
		],
	},
	...
	function replaceProgVars(elem,toSearch,toReplace)
	{
		...
		{
			if ((descr.fields[i].automatic_var_replacement!=undefined)&&(descr.fields[i].automatic_var_replacement==false))
			{
				continue;
			}
	...
	function changeVarName(parent,act_elem,new_val)
	{
		    ...
		    (act_elem.type==block_types.enum_var_elem))
		{
			...
			replaceProgVars(proj_elems.sub_elems[2],act_elem.values[0],new_val);	
		}
	}
	...
	function generateAddButtonBlock(elem,index)
	{
		...
		var button_text="";
		if (descr_lookup[parent.type].nested_blocks.length>1)
		{
			var num_elems_to_show=descr_lookup[parent.type].nested_blocks_exp.length;
			if ((parent.type==block_types.hmi_screens)&&(advanced_hmi_layout==false))
			{
				num_elems_to_show=1; // only show condition element;
				button_text="Add Screen";
			}
			else if ((is_func_block==true)&&(func_block_task==true)&&(basic_mode==true))
			{
		...
		if (fields[field_num].type==field_types.str_num)
		{
			...
			if (fixed==false)
			{
				...
				else
				{
					...
					{
						str+="<input "+key_down_event+focus_designator+"  style=\""+bg_color+"font-weight:"+font_weight+";color:"+color+";width:"+fields[field_num].width+"em\" onblur=\"ARGEE_elem_descr.handleEvent("+event.CHANGE+",this)\" type=\"text\" data-elem_type=\"str\" data-elem_index=\""+JSON.stringify(index)+"\" data-elem_field=\""+field_num+"\" value=\""+escapeHTML(val)+"\">";
					}
					else
					{
						str+="<input "+key_down_event+focus_designator+" style=\""+bg_color+"font-weight:"+font_weight+";color:"+color+";\" onblur=\"ARGEE_elem_descr.handleEvent("+event.CHANGE+",this)\" type=\"text\" data-elem_type=\"str\" data-elem_index=\""+JSON.stringify(index)+"\" data-elem_field=\""+field_num+"\" value=\""+escapeHTML(val)+"\">";									
					}
				}
			}
			else
	...
	function renderEditVars_imp(elem,index,parent)
	{
		...
		if (descr.screens.indexOf(screens.var_scr)!=-1)
		{
			...
			else if ((descr.render_init!=true)&&(elem.sub_elems!=undefined)&&(descr.nested_blocks.length==1))
			{

				
		
				
				if ((elem.type==block_types.funct_block_var)&&(elem.values[0].localeCompare(default_task_type)==0))
				{
				...
				if (elem.sub_elems!=undefined)
				{
					...
					else*/
					{
						...
						else if (elem.sub_elems.length>0)
						{
							...
							i=0;						
							var field2="<input "+focus_designator+" style=\"font-size:95%;\" onkeydown=\"return ARGEE_elem_descr.actOnKeyDown(event,this);\" onblur=\"ARGEE_elem_descr.handleEvent("+event.CHANGE+",this)\" type=\"text\" data-elem_type=\"str\" data-elem_index=\""+JSON.stringify(tmp)+"\" data-elem_field=\""+field_num+"\" value=\""+escapeHTML(elem.sub_elems[i].values[field_num])+"\">";
	...
	function handleElemOp(parent,type,index,op,val,elem_field)
	{
		...
		if (op==event.CHANGE)
		{
				 ...
				 
			{
				...
				var res;
				if ((res=checkIfVarChangeValid(parent,elem,val))==-1)
				{
	...
	function performUndo()
	{
		...
		{
			curr_undo_pos--;
			proj_elems.sub_elems=fastClone(undo_points[curr_undo_pos].sub_elems);
			setLocalStorage("prog_code",JSON.stringify(proj_elems));
			renderCombined(true);
		}
	}
	...
	function addUndo()
	{
		...
		curr_undo_pos++;
		undo_points[curr_undo_pos]=fastClone(proj_elems);
	...
	function handleEvent(act_event,elem)
	{
		...
		switch(act_event)
		{
			...
			case event.PASTE_ABOVE:
				 ...
				 {
					 act_elem=clipboard_buffer[last_clip_element_type];
					 ...
					 
					 {
						...
						parent.sub_elems.splice(last_index_elem,0,fastClone(clipboard_buffer[last_clip_element_type]));
			...
			case event.PASTE_BELOW:
				 ...
				 else
				 {
					 ...
					 if (descr_lookup[parent.type].nested_blocks.indexOf(last_clip_element_type)!=-1)
					 {
						...
						parent.sub_elems.splice(last_index_elem+1,0,fastClone(clipboard_buffer[last_clip_element_type]));
						if (descr_lookup[act_elem.type].custom_event_handle!=undefined){descr_lookup[act_elem.type].custom_event_handle(act_event,null,act_elem,null,null);}
					 }
				 }
				 break;
	...
	function preRenderProg(elem,index,parent,funct_block_index)
	{
		...
		if (descr.screens.indexOf(screens.prog_scr)!=-1)
		{
			...
			else
			{
				...
				else if (descr.type==block_types.function_block_group_def)
				{
					if (parseInt(parent.values[1])!=0)
					{
						str=parent.values[0];
				...
				if (descr.type==block_types.add_block)
				{
					if (rw==true)
					{
						var tmp=index.slice(0);
						tmp.splice(tmp.length,0,0);
						pre_render_buf[pre_render_buf.length]={elem:elem,descr:descr,index:tmp,str:str,render_border:false};
					}
				}
				else
				{
					var render_border;
					if ((elem.sub_elems!=undefined)||(descr.render_text_area==true))
					{
						render_border=true;
					}
					else
					...
					if ((descr.type==block_types.funct_block_def)||(descr.type==block_types.funct_def)||(descr.type==block_types.hmi_screens)||(descr.type==block_types.function_block_group_def))
					{
	...
	function postRenderProg()
	{
		...
		{
			str+="<tr>"
			if (((pre_render_buf[i].descr.type==block_types.funct_block_def)||(pre_render_buf[i].descr.type==block_types.hmi_screens)||(pre_render_buf[i].descr.type==block_types.function_block_group_def)))
			{
				if (i>0)
				{
					if ((prev_block_i!=-1)&&(pre_render_buf[prev_block_i].elem.expanded==false))
					{
						str+="<td colspan=\"2\" height=\"30px\" width=\"10px\" style=\"border-left: 1px solid transparent;border-top: 1px solid black;border-right: 0px solid transparent;border-bottom: 1px solid black;\" ><hr style=\"border-color:green;border-width:1px;\"></td>";
					}
					else
					{
						str+="<td colspan=\"2\" height=\"30px\" width=\"10px\" style=\"border-left: 1px solid transparent;border-top: 1px solid transparent;border-right: 0px solid transparent;\" ><hr style=\"border-color:green;border-width:1px;\"></td>";
					}
					str+="<td colspan=\"100%\" height=\"30px\" width=\"10px\" style=\"border-left: 0px solid transparent;border-top: 1px solid black;border-right: 1px solid transparent;\" ><hr style=\"border-color:green;border-width:1px;\"></td>";
					str+="</tr><tr>";
				}
				prev_block_i=i;
			}
			...
			else if (pre_render_buf[i].descr.type==block_types.add_block)
			{
				...
				pre_render_buf[i].index=pre_render_buf[i].index.slice(0,pre_render_buf[i].index.length-1);
			...
			else
			{
				...
				if (pre_render_buf[i].render_border==true)
				{
					...
					str+="<td id=\"prog_elem_2_"+line+"\" style=\"border-right: 1px solid transparent;white-space: nowrap; white-space: nowrap;padding-left:5px; padding-right:15px;\" width=\"1px\" bgcolor=\""+first_col_color+"\">"+pre_render_buf[i].descr.title+"</td>";
				...
				{
					str+="<td id=\"prog_elem_2_"+line+"\" style=\"white-space: nowrap;border: 1px solid black; white-space: nowrap;padding-left:5px; padding-right:15px;\" width=\"1px\" bgcolor=\""+first_col_color+"\">"+pre_render_buf[i].descr.title+"</td>";
	...
	function handleCollapseExpand(elem,parent,new_val,prog_only)
	{
		...
		if ((descr_lookup[elem.type].collapsable==true)&&(prog_only==false))
		{
			if (descr_lookup[elem.type].screens.indexOf(screens.var_scr)!=-1)
			{
				...
				else if ((elem.type==block_types.funct_block_var)&&(elem.values[1]==1))
				{
						elem.expanded=true;
				}
	...
	function setRendMode(mode)
	{
		if (mode==0)
		{
			...
			handleCollapseExpand(proj_elems,proj_elems,true,false);
		}
		else
		{
			rend_mode=REND_MODE.EXPANDED;
			handleCollapseExpand(proj_elems,proj_elems,false,false);
		}
		renderCombined(true);
	}
	...
	function renderEditVars()
	{
		...
		var_div.innerHTML="<h1>&nbsp&nbsp&nbsp&nbsp<u>Variables and Definitions</u> </h1><br><br>"+renderEditVars_imp(proj_elems,[],proj_elems)+"<br><br>";
	}
	...
	function renderProg()
	{
		...
		prog_div.innerHTML="<h1>&nbsp&nbsp&nbsp&nbsp<u>ARGEE Program</u></h1>"
		if (debug_mode==false)
		{
		...
		prog_div.innerHTML+="<br><div id=\"first_prog_line\"></div>"+postRenderProg();
		if (scroll_prop!=-1)
		{
	...
	function getCustElemDatabases(create)
	{
		...
		return {func:function_list_datapoints,local:local_io_datapoints,glob:combined_glob_io_alias_vars,enums:combined_enumerations,hmi_funcs:hmi_func_datapoints};
	}
	...
	[
		{group:"String/Arrays", 
				elements:[
							...
							["STR_CAT(source_str,dest_str) - String Concatinate","STR_CAT(,)"],
	...
	function gotoFuncBlock(elem)
	{
		...
		if (rend_mode==REND_MODE.COMPACT)
		{
			...
			handleCollapseExpand(proj_elems,proj_elems,true,true); 
		...
		if (GOM.getObjNum("DEV_RUN",0)==0)
		{
			...
			if (nst_line>=0)
			{
				...
				else
				{
					...
					{
						setCompilerMessage(false,true,"Exception detected in the Program item: "+ (argee_line));
