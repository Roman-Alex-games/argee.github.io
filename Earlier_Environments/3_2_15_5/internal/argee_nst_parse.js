...
function setErrMsg(e)
{
	...
	{
		setCompilerMessage(false,true,"Error: "+(e.err_msg));
	}
}
...
var token_types=
{
	...
	*/
	...
	HMI_TABLE_SCREEN:700+6,
	END_HMI_TABLE_SCREEN:700+7,
	HMI_TABLE:700+8,
	END_HMI_TABLE:700+9,
	HMI_TABLE_ROW:700+10,
	HMI_TABLE_COL:700+11,
	END_HMI_TABLE_ROW:700+12,
	END_HMI_TABLE_COL:700+13,
...
// all other token symbols	
	...
	{elem:token_types.HMI_TABLE_SCREEN,sym:"HMI_TABLE_SCREEN"},
	{elem:token_types.END_HMI_TABLE_SCREEN,sym:"END_HMI_TABLE_SCREEN"},
	{elem:token_types.HMI_TABLE,sym:"HMI_TABLE"},
	{elem:token_types.END_HMI_TABLE,sym:"END_HMI_TABLE"},

	{elem:token_types.HMI_TABLE_ROW,sym:"HMI_TABLE_ROW"},
	{elem:token_types.HMI_TABLE_COL,sym:"HMI_TABLE_COL"},
	{elem:token_types.END_HMI_TABLE_ROW,sym:"END_HMI_TABLE_ROW"},
	{elem:token_types.END_HMI_TABLE_COL,sym:"END_HMI_TABLE_COL"},
	...
	while(1)
	{
		...
		else if (next_token==token_types.HMI_BEGIN)
		{
			...
			while (next_token!=token_types.END_HMI)
			{
				...
				else if (next_token==token_types.HMI_TABLE_SCREEN)
				{
					screen=new Object();
					setObjType(screen,"HMI_TABLE_SCREEN",ELEM.HMI_TABLE_SCREEN);
					getNextToken(); // switch to name
					screen.name=clone(next_token_suppliment);
					getNextToken();
					var curr_elem=0;
					screen.extra_elements=[];
					while(next_token==token_types.COMMA)
					{
						getNextToken();
						screen.extra_elements[curr_elem]=clone(next_token_suppliment);
						curr_elem++;
						getNextToken();
					}
					match(token_types.SEMICOL);
					screen.list=[];
					//getNextToken();
					while(next_token!=token_types.END_HMI_TABLE_SCREEN)
					{
						var table;
						if (next_token!=token_types.HMI_TABLE)
						{
							stopCompilation("Invalid token in HMI definition");
						}
						table=new Object();
						setObjType(table,"HMI_TABLE",ELEM.HMI_TABLE);
						getNextToken(); // switch to name
						table.name="";
						var curr_elem=0;
						table.extra_elements=[];
						table.extra_elements[curr_elem]=clone(next_token_suppliment);curr_elem++;
						getNextToken();
						while(next_token==token_types.COMMA)
						{
							getNextToken();
							table.extra_elements[curr_elem]=clone(next_token_suppliment);
							curr_elem++;
							getNextToken();
						}
						match(token_types.SEMICOL);
						...
						table.list=[];
						//getNextToken(); 
						while (next_token!=token_types.END_HMI_TABLE)
						{
							var row;
							if (next_token!=token_types.HMI_TABLE_ROW)
							{
								stopCompilation("Invalid token in HMI definition");
							}						
							row=new Object();
							setObjType(row,"HMI_TABLE_ROW",ELEM.HMI_TABLE_ROW);
							getNextToken(); // switch to name
							row.name="";
							var curr_elem=0;
							row.extra_elements=[];
							row.extra_elements[curr_elem]=clone(next_token_suppliment);curr_elem++;
							getNextToken();
							while(next_token==token_types.COMMA)
							{
								getNextToken();
								row.extra_elements[curr_elem]=clone(next_token_suppliment);
								curr_elem++;
								getNextToken();
							}
							match(token_types.SEMICOL);
							...
							row.list=[];
							//getNextToken(); 
							while (next_token!=token_types.END_HMI_TABLE_ROW)
							{
								var col;
								if (next_token!=token_types.HMI_TABLE_COL)
								{
									stopCompilation("Invalid token in HMI definition");
								}						
								col=new Object();
								setObjType(col,"HMI_TABLE_COL",ELEM.HMI_TABLE_COL);
								getNextToken(); // switch to name
								col.name="";
								var curr_elem=0;
								col.extra_elements=[];
								col.extra_elements[curr_elem]=clone(next_token_suppliment);curr_elem++;
								getNextToken();
								while(next_token==token_types.COMMA)
								{
									getNextToken();
									col.extra_elements[curr_elem]=clone(next_token_suppliment);
									curr_elem++;
									getNextToken();
								}
								match(token_types.SEMICOL);
								...
								col.list=[];
								//getNextToken(); 
								while (next_token!=token_types.END_HMI_TABLE_COL)
								{
									var obj_func=parseVariableOrFunction(null,true,true);
									col.list[col.list.length]=obj_func;
									match(token_types.SEMICOL);
								}
								getNextToken(); 
								row.list[row.list.length]=col;
							}
							getNextToken(); 
							table.list[table.list.length]=row;
						}
						getNextToken(); 
						screen.list[screen.list.length]=table;
					}
				}
				else
				{
...
function parseVarSegm(global)
{
	...
	while(next_token!=token_types.END_VAR)
	{
		...
		{
			stopCompilation("Identifier expected",obj1);
		}
		if (next_token==token_types.LEFT_ROUND_BRACKET)
		{
			// comment
